    ...
    <nav className="flex max-md:hidden justify-between items-center gap-8">
      {links.map((link) => (
        ...
        >
          ...
          {link.path === path && (
            <motion.span
              key={`underline-${link.path}`} // Ajout de clé unique pour l'animation
          ...
          <motion.span
            key={`hover-line-${link.path}`} // Ajout de clé unique pour l'animation
            whileHover={{ scaleX: 1 }}
            whileFocus={{ scaleX: 1 }}
            initial={{ scaleX: 0 }}
            transition={{ type: "tween", duration: 0.3 }}
