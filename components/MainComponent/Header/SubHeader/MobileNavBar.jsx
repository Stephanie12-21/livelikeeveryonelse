      ...
      <div className="flex flex-col items-center gap-24 container p-4 md:px-8">
        <nav className="flex flex-col mt-32 items-center gap-16">
          {links.map((link) => (
            <Link
              href={link.path}
              key={link.path} // Utilisation de `path` comme clé unique
            ...
            >
              ...
              {link.path === path && (
                <motion.span
                  ...
                  layoutId="underline"
                  key={`underline-${link.path}`} // Clé unique pour le span
              ...
              <motion.span
                ...
                initial={{ scaleX: 0 }}
                transition={{ type: "tween", duration: 0.3 }}
                key={`hover-line-${link.path}`} // Clé unique pour le span d'animation
                className="absolute left-0 bottom-0 w-full h-[3px] bg-white"
              />
            </Link>
          ))}
